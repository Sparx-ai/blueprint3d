    ...
    (function (Model) {
        ...
        var Scene = (function () {
            ...
            function Scene(model, textureDir) {
                ...
                // init GLB/GLTF item loader
                this.loader = new THREE.GLTFLoader();
                // Note: GLTFLoader doesn't have crossOrigin property like JSONLoader
            }
             ...
             * @param fileName The name of the file to load (should be .glb or .gltf).
             * @param metadata TODO
            ...
            Scene.prototype.addItem = function (itemType, fileName, metadata, position, rotation, scale, fixed) {
                itemType = itemType || 1;
                var scope = this;
                // GLTFLoader callback signature is different - receives GLTF object
                var loaderCallback = function (gltf) {
                    console.log('GLB Item loaded:', fileName, gltf);
                    // Extract the main scene from the GLTF
                    var loadedScene = gltf.scene;
                    // Create geometry and materials from the loaded model
                    // For now, we'll use the first mesh we find
                    var geometry = new THREE.BoxGeometry(50, 50, 50); // Default fallback
                    var materials = [];
                    var foundGeometry = false;
                    loadedScene.traverse(function (child) {
                        if (child.isMesh || (child instanceof THREE.Mesh) || child.type === 'Mesh') {
                            if (!foundGeometry && child.geometry) {
                                // Convert BufferGeometry to Geometry if needed for compatibility
                    ...
                    // Log if we used fallback geometry
                    if (!foundGeometry) {
                        console.warn('No geometry found in GLB file, using fallback box geometry');
                    }
                    // Fallback: if no materials found, create a basic material
                    if (materials.length === 0) {
                        console.warn('No materials found in GLB file, creating fallback material');
                        materials = [new THREE.MeshBasicMaterial({ color: 0xff0000 })];
                    }
                    // Create the item using the extracted geometry and materials
                    var item = new (BP3D.Items.Factory.getClass(itemType))(scope.model, metadata, geometry, materials, // Use array directly instead of MeshFaceMaterial
                ...
                this.loader.load(fileName, loaderCallback, function (progress) {
                    console.log('Loading progress:', fileName, (progress.loaded / progress.total * 100) + '%');
                }, function (error) {
